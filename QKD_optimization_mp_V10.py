...
5) trying to calculate SKR from PTM of channel... no plotting at this moment
6) absence of reference frame now!
7) Direct + estimation both available now...
8) plotting
"""
...
def rand_U_direct(prob, seed=42, n_samples = None, verbose = True):
    """
    Directly constructing M, then calculating the skr. No estimation.
...
def compute_for_parameters(eps, seed, a, b, c, n_samples=1000):
    """
    Compute results for a given epsilon and seed.
    """
    ...
    seed = int(round(eps*100*seed))
    try:
        # Asymptotic estimation (n_samples=None)
        res_asymptotic = rand_U_direct(prob, seed=seed, n_samples=None)
        
        # Finite sample estimation
        res_estimation = rand_U_direct(prob, seed=seed, n_samples=n_samples)
        
        # Unoptimized QKD
        res_unoptimized = no_opt_QKD(prob, seed=seed)
        
        return {
            'eps': eps,
            'seed': seed,
            # Asymptotic results
            'asymptotic_skr_bb84': res_asymptotic.get('skr_bb84', np.nan),
            'asymptotic_skr_ss': res_asymptotic.get('skr_ss', np.nan),
            'asymptotic_Qz': res_asymptotic.get('Qz', np.nan),
            'asymptotic_Qx': res_asymptotic.get('Qx', np.nan),
            'asymptotic_Qy': res_asymptotic.get('Qy', np.nan),
            # Finite sample results
            'estimation_skr_bb84': res_estimation.get('skr_bb84', np.nan),
            'estimation_skr_ss': res_estimation.get('skr_ss', np.nan),
            'estimation_Qz': res_estimation.get('Qz', np.nan),
            'estimation_Qx': res_estimation.get('Qx', np.nan),
            'estimation_Qy': res_estimation.get('Qy', np.nan),
            # Unoptimized results
            'unopt_skr_bb84': res_unoptimized.get('skr_bb84', np.nan),
            'unopt_skr_ss': res_unoptimized.get('skr_ss', np.nan),
            'unopt_Qz': res_unoptimized.get('Qz', np.nan),
            'unopt_Qx': res_unoptimized.get('Qx', np.nan),
            'unopt_Qy': res_unoptimized.get('Qy', np.nan),
        }
        ...
        return {
            'eps': eps,
            'seed': seed,
            'asymptotic_skr_bb84': np.nan,
            'asymptotic_skr_ss': np.nan,
            'asymptotic_Qz': np.nan,
            'asymptotic_Qx': np.nan,
            'asymptotic_Qy': np.nan,
            'estimation_skr_bb84': np.nan,
            'estimation_skr_ss': np.nan,
            'estimation_Qz': np.nan,
            'estimation_Qx': np.nan,
            'estimation_Qy': np.nan,
            'unopt_skr_bb84': np.nan,
            'unopt_skr_ss': np.nan,
            'unopt_Qz': np.nan,
            'unopt_Qx': np.nan,
            'unopt_Qy': np.nan,
...
def run_simulations(epss_v, seeds, a=0.5, b=0.3, c=0.2, n_samples=1000, n_workers=4):
    """
    Run simulations over epsilon values and seeds using multiprocessing.
    """
    print(f"\n{'='*60}")
    print(f"Running QKD Simulations")
    print(f"{'='*60}")
    print(f"Epsilon values: {len(epss_v)}")
    print(f"Seeds: {len(seeds)}")
    print(f"Total simulations: {len(epss_v) * len(seeds)}")
    print(f"Using {n_workers} workers")
    
    # Create parameter combinations
    param_list = [(eps, seed, a, b, c, n_samples) for eps in epss_v for seed in seeds]
    
    # Run parallel processing
    with mp.Pool(processes=n_workers) as pool:
        results = pool.starmap(compute_for_parameters, tqdm(param_list, desc="Simulations"))
    
    # Convert to numpy array with all Q values included
    dtype = [
        ('eps', float),
        ('seed', int),
        ('asymptotic_skr_bb84', float),
        ('asymptotic_skr_ss', float),
        ('asymptotic_Qz', float),
        ('asymptotic_Qx', float),
        ('asymptotic_Qy', float),
        ('estimation_skr_bb84', float),
        ('estimation_skr_ss', float),
        ('estimation_Qz', float),
        ('estimation_Qx', float),
        ('estimation_Qy', float),
        ('unopt_skr_bb84', float),
        ('unopt_skr_ss', float),
        ('unopt_Qz', float),
        ('unopt_Qx', float),
        ('unopt_Qy', float),
    ...
    data_array = np.array([tuple(res.values()) for res in results], dtype=dtype)
    
    return data_array

def plot_results(data_array, epss_v, save_plot=True):
    """
    Simple plot of average key rates.
    """
    ...
    for eps in epss_v:
        mask = data_array['eps'] == eps
        eps_data = data_array[mask]
        
        # BB84
        means_bb84.append([
            np.nanmean(eps_data['asymptotic_skr_bb84']),
            np.nanmean(eps_data['estimation_skr_bb84']),
            np.nanmean(eps_data['unopt_skr_bb84'])
        ])
        stds_bb84.append([
            np.nanstd(eps_data['asymptotic_skr_bb84']),
            np.nanstd(eps_data['estimation_skr_bb84']),
            np.nanstd(eps_data['unopt_skr_bb84'])
        ])
        
        # Six-state
        means_ss.append([
            np.nanmean(eps_data['asymptotic_skr_ss']),
            np.nanmean(eps_data['estimation_skr_ss']),
            np.nanmean(eps_data['unopt_skr_ss'])
        ])
        stds_ss.append([
            np.nanstd(eps_data['asymptotic_skr_ss']),
            np.nanstd(eps_data['estimation_skr_ss']),
            np.nanstd(eps_data['unopt_skr_ss'])
        ])
    
    means_bb84 = np.array(means_bb84)
    stds_bb84 = np.array(stds_bb84)
    ...
    # BB84 plot
    x = np.arange(len(epss_v))
    width = 0
    ax1.errorbar(x - width, means_bb84[:, 0], yerr=stds_bb84[:, 0], 
                 fmt='o-', label='Asymptotic', capsize=3)
    ax1.errorbar(x, means_bb84[:, 1], yerr=stds_bb84[:, 1], 
                 fmt='s--', label='Estimation', capsize=3)
    ax1.errorbar(x + width, means_bb84[:, 2], yerr=stds_bb84[:, 2], 
                 fmt='^-', label='Unoptimized', capsize=3)
    ax1.set_xlabel('Channel fidelity (ε)')
    ax1.set_ylabel('Secret Key Rate')
    ax1.set_title('BB84 Protocol')
    ax1.set_xticks(x)
    ax1.set_xticklabels([f'{eps:.2f}' for eps in epss_v], rotation=45)
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ax1.set_ylim(0, 1)
    
    # Six-state plot
    ax2.errorbar(x - width, means_ss[:, 0], yerr=stds_ss[:, 0], 
                 fmt='o-', label='Asymptotic', capsize=3)
    ax2.errorbar(x, means_ss[:, 1], yerr=stds_ss[:, 1], 
                 fmt='s--', label='Estimation', capsize=3)
    ax2.errorbar(x + width, means_ss[:, 2], yerr=stds_ss[:, 2], 
                 fmt='^-', label='Unoptimized', capsize=3)
    ax2.set_xlabel('Channel fidelity (ε)')
    ax2.set_ylabel('Secret Key Rate')
    ax2.set_title('Six-State Protocol')
    ax2.set_xticks(x)
    ax2.set_xticklabels([f'{eps:.2f}' for eps in epss_v], rotation=45)
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    ax2.set_ylim(0, 1)
    
    
    # Comparison plot: BB84 vs Six-State (Asymptotic only for clarity)
    ax3.errorbar(x - width/2, means_bb84[:, 0], yerr=stds_bb84[:, 0], 
                 fmt='o-', label='BB84 Asymptotic', capsize=3, color='blue')
    ax3.errorbar(x + width/2, means_ss[:, 0], yerr=stds_ss[:, 0], 
                 fmt='s--', label='Six-State Asymptotic', capsize=3, color='red')
    ax3.set_xlabel('Channel fidelity (ε)')
    ax3.set_ylabel('Secret Key Rate')
    ax3.set_title('Comparison: BB84 vs Six-State')
    ax3.set_xticks(x)
    ax3.set_xticklabels([f'{eps:.2f}' for eps in epss_v], rotation=45)
    ax3.legend()
    ...
    if save_plot:
        ...
        print(f"\nPlot saved as 'qkd_results.png'")
        
        # Generate filename with timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f'data_files/skr_summary_{timestamp}.dat'
        
        # Prepare data matrix
        summary_data = np.column_stack([
            epss_v,
            means_bb84[:, 0], stds_bb84[:, 0],
            means_bb84[:, 1], stds_bb84[:, 1],
            means_bb84[:, 2], stds_bb84[:, 2],
            means_ss[:, 0], stds_ss[:, 0],
            means_ss[:, 1], stds_ss[:, 1],
            means_ss[:, 2], stds_ss[:, 2]
        ])
        
        # Header string
        header = 'eps\tskr_bb84_asym_mean\tskr_bb84_asym_std\tskr_bb84_est_mean\tskr_bb84_est_std\tskr_bb84_unopt_mean\tskr_bb84_unopt_std\tskr_ss_asym_mean\tskr_ss_asym_std\tskr_ss_est_mean\tskr_ss_est_std\tskr_ss_unopt_mean\tskr_ss_unopt_std'
        
        # Save with tab delimiter, no comment character
        np.savetxt(filename, summary_data, delimiter='\t', header=header, comments='', fmt='%.8f')
...
if __name__ == "__main__":
    ...
    # Weights for non-uniform Pauli errors
    a, b, c = 1/3, 1/3, 1/3
    # a, b, c = 0.99,0.005,0.005
    
    # Run simulations
    data_array = run_simulations(
        epss_v=epss_v,
        seeds=seeds,
        a=a, b=b, c=c,
        n_samples=n_samples,
    ...
    # print(data_array)
    # Plot results
    plot_results(data_array, epss_v, save_plot=True)
    
    # Save data array
    # np.save('qkd_simulation_data.npy', data_array)
    # print(f"\nData saved to 'qkd_simulation_data.npy'")
    os.makedirs('data_files', exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename  = f"data_files/full_data_{timestamp}.dat"

    header = '\t'.join(data_array.dtype.names)

    np.savetxt(filename,
               np.column_stack([data_array[name] for name in data_array.dtype.names]),
               delimiter='\t',
               header=header,
               comments='',
               fmt='%.8e')
    ...
    # print(f"Columns: {data_array.dtype.names}")
    ######### Plotting
    # Set LaTeX fonts
    plt.rcParams.update({
        'font.family': 'serif',
    ...
    data = np.column_stack([data_array[name] for name in data_array.dtype.names])


    # Extract columns
    Qz = data[:, 4]  # Qz_opt_bb84 column
    Qx = data[:, 5]  # Qx_opt_bb84 column
    skr = skr_BB84(Qz, Qx)
    skr_exp = data[:, 2]
    eps_v = data[:, 0]
    ...
    grid_size = 1000
    Qz_grid = np.linspace(0, 0.5, grid_size)
    Qx_grid = np.linspace(0, 0.5, grid_size)
    Qz_mesh, Qx_mesh = np.meshgrid(Qz_grid, Qx_grid)
    skr_mesh = skr_BB84(Qz_mesh, Qx_mesh)

    # Contour plot with levels from 0 to 1
    contour = ax.contourf(Qz_mesh, Qx_mesh, skr_mesh, 
                          levels=np.linspace(0, 1, 101), 
                          cmap='binary', vmin = 0, vmax = 1)
    # Add contour line at SKR = 0
    #ax.contour(Qz_mesh, Qx_mesh, skr_mesh, levels=[0], colors='darkgreen', linewidths=2)
    
    #cs = ax.contour(Qz_mesh, Qx_mesh, skr_mesh, levels=[0], colors='darkgreen', linewidths=2)
    cs = ax.contour(Qz_mesh, Qx_mesh, skr_mesh, levels=[0], colors='darkgreen', linewidths=2)
    # skip clabel entirely, place annotation manually
    ax.annotate(r'$R_{\mathrm{BB84}} = 0$',
                xy=(0.30, 0.015),            # point on the contour line
                xytext=(0.32, 0.05),        # label position above the line
                fontsize=11,
                color='darkgreen',
                rotation=0,
                xycoords='data',
    ...
    # Scatter with eps_v as color
    scatter = ax.scatter(Qz, Qx, c=eps_v, cmap='plasma', s=20, edgecolor='blue', linewidth=0.5)

    ax.set_xlabel('$Q_z$ (BB84 opt)')
    ax.set_ylabel('$Q_x$ (BB84 opt)')

    # Add two colorbars
    # cbar1 = plt.colorbar(contour, ax=ax, label='Secret Key Rate (bits/sifted bit)', location='right')
    cbar1 = plt.colorbar(contour, ax=ax, label='Secret Key Rate (bit/sifted bit)', location='right', 
                         ticks=np.linspace(0, 1, 6))
    cbar1.set_ticklabels([f'{x:.1f}' for x in np.linspace(0, 1, 6)])
    cbar2 = plt.colorbar(scatter, ax=ax, 
                         label='Channel Parameter $\\epsilon$', 
                         location='left', pad = 0.15)

    plt.savefig('BB84_opt_new.pdf', format='pdf', dpi=600, bbox_inches='tight')
    plt.show()

    # Extract columns
    Qz = data[:, -3]  # Qz_std column
    Qx = data[:, -2]  # Qx_std column
    skr = skr_BB84(Qz, Qx)
    eps_v = data[:, 0]
    ...
    # Contour plot with levels from 0 to 1
    contour = ax.contourf(Qz_mesh, Qx_mesh, skr_mesh, 
                          levels=np.linspace(0, 1, 101), cmap='binary')
    # Add contour line at SKR = 0
    # ax.contour(Qz_mesh, Qx_mesh, skr_mesh, levels=[0], colors='darkgreen', linewidths=2)
    cs = ax.contour(Qz_mesh, Qx_mesh, skr_mesh, levels=[0], colors='darkgreen', linewidths=2)
    # skip clabel entirely, place annotation manually
    ...
    ax.set_xlabel('$Q_z$ (BB84 std)')
    ax.set_ylabel('$Q_x$ (BB84 std)')
    ...
    cbar1 = plt.colorbar(contour, ax=ax, label='Secret Key Rate (bit/sifted bit)', location='right', 
                         ticks=np.linspace(0, 1, 6))
    cbar1.set_ticklabels([f'{x:.1f}' for x in np.linspace(0, 1, 6)])
    cbar2 = plt.colorbar(scatter, ax=ax, label='Channel Parameter $\\epsilon$ ',
                         location='left', pad = 0.15)

    plt.savefig('BB84_std_new.pdf', format='pdf', dpi=600, bbox_inches='tight')
    plt.show()
