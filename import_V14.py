...
Difference from V12: Plotting the actual (achieved) SKR as well
"""

import numpy as np
from QKD_DE_V10 import rand_U_direct, no_opt_QKD, skr_BB84, H
...
finite_means_ss = []
finite_rmse_ss = []
finite_means_bb84_act = [] # actual/achieved on channels
finite_rmse_bb84_act = []
finite_means_ss_act = []
finite_rmse_ss_act = []
...
for n in n_samples:
    ...
    for i, seed in enumerate(seeds):
        ...
        Q1, Q2, Q3 = 1-res['Qz'], 1-res['Qx'], 1-res['Qy']
        Q_s = sorted([Q1, Q2, Q3])
        l00 = 1 - (Q1 + Q2 + Q3)/2
        l01 = (Q1 + Q2 - Q3)/2
        l10 = (-Q1 + Q2 + Q3)/2
        ...
        skr_bb84_list_act.append(skr_BB84(Q_s[0], Q_s[1]))
        skr_ss_list_act.append(skr_six)
        # print(Q1, Q2, Q3)
        
        # Error for this seed: (true - estimated)^2
        errors_bb84.append((asym_skr_bb84[i] - res['skr_bb84'])**2)
        errors_bb84_act.append((asym_skr_bb84[i] - skr_BB84(Q_s[0], Q_s[1]))**2)
        errors_ss.append((asym_skr_ss[i] - res['skr_ss'])**2)
        errors_ss_act.append((asym_skr_ss[i] - skr_six)**2)
    
    # Mean of finite sample estimates
    finite_means_bb84.append(np.mean(skr_bb84_list))
    finite_means_ss.append(np.mean(skr_ss_list))
    finite_means_bb84_act.append(np.mean(skr_bb84_list_act))
    finite_means_ss_act.append(np.mean(skr_ss_list_act))
    
    # RMSE: sqrt of mean squared errors
    finite_rmse_bb84.append(np.sqrt(np.mean(errors_bb84)))
    finite_rmse_bb84_act.append(np.sqrt(np.mean(errors_bb84_act)))
    finite_rmse_ss.append(np.sqrt(np.mean(errors_ss)))
    finite_rmse_ss_act.append(np.sqrt(np.mean(errors_ss_act)))
...
ax1.errorbar(n_samples, finite_means_bb84_act, 
             yerr=finite_rmse_bb84_act, 
             fmt='gx-.', capsize=5, label='Achieved (RMSE)')
ax1.set_xscale('log')
...
ax2.errorbar(n_samples, finite_means_ss, yerr=finite_rmse_ss, 
             fmt='ro-', capsize=5, label='Finite sample (RMSE)')
ax2.errorbar(n_samples, finite_means_ss_act, 
             yerr=finite_rmse_ss_act, 
             fmt='gx-.', capsize=5, label='Achieved (RMSE)')
ax2.set_xscale('log')
